...
class Migrasi_data_opt extends CI_Controller {
  ...
  function ambil_finger_masuk_pulang($date_mulai, $date_akhir, $id_pegawai)
  {
    $finger =  $this->_ci->db->query("
        SELECT
            t_masuk.tanggal_masuk, t_keluar.tanggal_keluar
        FROM
        (
          SELECT
            to_char(min(tanggal),'yyyy-mm-dd HH24:MI') as tanggal_masuk
          FROM
            absensi_log
          WHERE
            tanggal BETWEEN '".$date_mulai."' AND '".$date_akhir."' and
            badgenumber||id_mesin in (SELECT user_id||id_mesin from mesin_user WHERE id_pegawai = '".$id_pegawai."')
        ...
        (
          SELECT
            to_char(max(tanggal),'yyyy-mm-dd HH24:MI') as tanggal_keluar
          FROM
            absensi_log
          WHERE
            tanggal BETWEEN '".$date_mulai."' AND '".$date_akhir."' and
          ...
          GROUP BY to_char(tanggal, 'yyyy-mm-dd')
          order by
            to_char(tanggal, 'yyyy-mm-dd') desc
        ) t_keluar on t_masuk.tanggal_masuk::date = t_keluar.tanggal_keluar::date")->row();

        if ($finger == null)
        {
          $finger = new stdClass();
          $finger->tanggal_masuk = null;
          $finger->tanggal_keluar = null;
        }
  ...
  function cek_jumlah_finger_rev($date_mulai, $date_akhir, $id_pegawai){
    return $this->_ci->db->query("select
              count(distinct(tanggal))  as jumlah
            from
              absensi_log
            where
              tanggal BETWEEN '".$date_mulai."' and '".$date_akhir."'
  ...
  function cek_ulang_data_mentah($date, $id_pegawai, $fungsi, $tampil, $lembur, $izin, $jadwal, $meninggal = null, $tgl_meninggal = null){
    ...
    $insert = true;
   /* //kosongkan id jam kerja jika ada libur
    if ($jadwal['id_hari_libur'] <> null) {
      $jadwal['id_jam_kerja'] = null;
    }*/
    ...
    //end cek ijin
      ...
      if($jadwal["jkr_id_jam_kerja"] <> null) {
        ...
        if($jadwal["jkr_kode_role_jam_kerja"] == "LB") {
          ...
          if($lembur <> null) {
            //$data_jumlah_finger = $this->cek_jumlah_finger_rev($date." 00:01", $date." 23:59", $id_pegawai);
            if($jadwal['jumlah_finger'] > 1) {
              // $finger_masuk   = $dt_finger_masuk;
              // $finger_pulang  = $dt_finger_pulang;
              $selisih_finger = $this->ambil_selisih_menit($jadwal['finger_masuk'], $jadwal['finger_pulang'], $id_pegawai);
              ...
              //nindes aturan tidak finger tapi upload lembur
              $menitLembur            = 0;
              $menitLemburDiakui      = 0;
              ...
              //nindes aturan tidak finger tapi upload lembur
              $keterangan             = "LIBUR ROSTER UPLOAD SURAT LEMBUR TIDAK FINGER";
            }
          }
          else{
            //tidak diakui lembur karena tanpa surat dan dihari ketika roster libur
            if($jadwal['jumlah_finger'] > 1) {
              // $finger_masuk   = $dt_finger_masuk;
              // $finger_pulang  = $dt_finger_pulang;
              $selisih_finger = $this->ambil_selisih_menit($jadwal['finger_masuk'], $jadwal['finger_pulang'], $id_pegawai);
          ...
          // $finger_roster = $this->ambil_finger_masuk_pulang($scanMulaiMasuk, $scanAkhirPulang, $id_pegawai);
          // $finger_masuk = $finger_roster->tanggal_masuk;
          // $finger_pulang = $finger_roster->tanggal_keluar;
          ...
          //////////// jika ada finger masuk dan pulang sesuai data Master Roster
            ...
            if($lembur <> null) {
              ...
              if(strtotime($finger_pulang) <  strtotime($date." ".$jadwal["jkr_jam_akhir_scan_pulang"]) && strtotime($finger_pulang) > strtotime($date." ".$jadwal["jkr_jam_pulang"])) {
                ...
                //nindes aturan tidak finger tapi upload lembur
                $keterangan           = "HADIR ROSTER DI HARI $nama_hari LEMBUR UPLOAD SURAT TANPA FINGER";
              }
        ...
        if($jadwal["id_hari_libur"] <> null) {
          ...
          if($lembur <> null) {
            // if data lembur
            if ($lembur['pulang_besoknya'] == 't') {
              $finger_masuk  = $this->ambil_finger_masuk($lembur['jam_lembur_awal'], $lembur['jam_lembur_akhir'], $id_pegawai)->tanggal;
              $finger_pulang = $this->ambil_finger_pulang($lembur['jam_lembur_awal'], $lembur['jam_lembur_akhir'], $id_pegawai)->tanggal;
              if ($finger_masuk != $finger_pulang) {
                $jadwal['jumlah_finger'] = 2;
              }
            }else{
              $finger_masuk   = $jadwal['finger_masuk'];
              $finger_pulang  = $jadwal['finger_pulang'];
            }
            //$data_jumlah_finger = $this->cek_jumlah_finger_rev($date." 00:01", $date." 23:59", $id_pegawai);
            if($jadwal['jumlah_finger'] > 1) {
              ...
              //nindes aturan tidak finger tapi upload lembur
              $keterangan             = "UPLOAD LEMBUR ".$jadwal["keterangan"]." TANPA FINGER";
            }
          } // end if data lembur
          else {
            //tidak diakui lembur karena tanpa surat dan dihari libur
            if($jadwal['jumlah_finger'] > 1) {
              $finger_masuk   = $jadwal['finger_masuk'];
              $finger_pulang  = $jadwal['finger_pulang'];
              $selisih_finger = $this->ambil_selisih_menit($finger_masuk, $finger_pulang, $id_pegawai);
              ...
              $keteranganTidakMasuk   = "";
              $jamKerja               = $jadwal["keterangan"];
              $keterangan             = "LEMBUR ".$jadwal["keterangan"]." TANPA SURAT SESUAI FINGER";
            } //end if jumlah data finger > 1
            else {
              $jadwalMasuk            = "";
              $jadwalPulang           = "";
            ...
            if(!$jadwal["id_jam_kerja"]) {
              if($lembur <> null){ //if data lembur
                ...
                if($jadwal['jumlah_finger'] > 1) {
                  ...
                  //nindes aturan tidak finger tapi upload lembur
                  $keterangan             = "UPLOAD LEMBUR HARI SABTU TIDAK FINGER";
                }
              } //end if data lembur
              else{
              ...
              if($jadwal['jumlah_finger'] < 1) {
                ...
                // $selisih_finger      = $this->ambil_selisih_menit($finger_masuk, $finger_pulang, $id_pegawai);
                // $finger_sabtu = $this->ambil_finger_masuk_pulang($date." ".$jadwal["jam_mulai_scan_masuk"], $date." ".$jadwal["jam_akhir_scan_pulang"], $id_pegawai);
                // $finger_masuk = $finger_sabtu->tanggal_masuk;
                // $finger_pulang = $finger_sabtu->tanggal_keluar;
                $selisih_finger = $this->ambil_selisih_menit($finger_masuk, $finger_pulang, $id_pegawai);
                ...
                if($lembur <> null) {
                  if(strtotime($finger_pulang) <  strtotime($date." ".$jadwal["jam_akhir_scan_pulang"]) && strtotime($finger_pulang) > strtotime($date." ".$jadwal["jam_pulang"])){
                    ...
                    //nindes aturan tidak finger tapi upload lembur
                    $keterangan           = "HADIR DI HARI $nama_hari UPLOAD LEMBUR TANPA FINGER";
                  }
                }
                else {
                  if(strtotime($finger_pulang) > strtotime($date." ".$jadwal["jam_pulang"])) {
          ...
          //jika hari minggu
            ...
            if(!$jadwal["id_jam_kerja"]) {
              ...
              if($lembur <> null) {
                ...
                if($jadwal['jumlah_finger'] > 1) {
                  ...
                  //nindes aturan tidak finger tapi upload lembur
                  $keterangan             = "UPLOAD LEMBUR HARI MINGGU TANPA FINGER";
                }
              }
              else {
                //jika tanpa surat lembur dan jumlah finger lebih dari 1
                if($jadwal['jumlah_finger'] > 1) {
                  // $finger_masuk    = $this->ambil_finger_masuk($date." 00:01", $date." 23:59", $id_pegawai)->tanggal;
                  // $finger_pulang  = $this->ambil_finger_pulang($date." 00:01", $date." 23:59", $id_pegawai)->tanggal;
                  // $selisih_finger  = $this->ambil_selisih_menit($finger_masuk, $finger_pulang, $id_pegawai);
                  ...
                  //tidak diakui lembur karena tanpa surat dan dihari minggu
                  $jadwalMasuk            = "";
                  $jadwalPulang           = "";
                  $menitPulangCepat       = "0";
                  $fingerMasuk            = $finger_masuk;
                  ...
                  $keteranganTidakMasuk   = "";
                  $jamKerja               = "";
                  $keterangan             = "LEMBUR HARI MINGGU TANPA SURAT SESUAI FINGER";
                }else{
                  //jumlahfinger 1 atau kurang dianggap libur di hari minggu
                  $fingerMasuk            = "";
                  $fingerPulang           = "";
                  $jadwalMasuk            = "";
                  $jadwalPulang           = "";
            ...
            //jika ada jadwal kerja
              ...
              if($jadwal['jumlah_finger'] < 1) {
                ...
                $finger_pulang       = $this->ambil_finger_pulang($date." ".$jadwal["jam_mulai_scan_masuk"], $date." ".$jadwal["jam_akhir_scan_pulang"], $id_pegawai)->tanggal;
                // $selisih_finger      = $this->ambil_selisih_menit($finger_masuk, $finger_pulang, $id_pegawai);
                //$finger_minggu = $this->ambil_finger_masuk_pulang($date." ".$jadwal["jam_mulai_scan_masuk"], $date." ".$jadwal["jam_akhir_scan_pulang"], $id_pegawai);
                //$finger_masuk = $finger_minggu->tanggal_masuk;
                //$finger_pulang = $finger_minggu->tanggal_keluar;
                $selisih_finger = $this->ambil_selisih_menit($finger_masuk, $finger_pulang, $id_pegawai);

                // jika finger masuk lebih besar dari jadwal masuk / telat
                if(strtotime($finger_masuk) > strtotime($date." ".$jadwal["jam_masuk"]) ){
                  $menit_telat       = $this->jumlah_menit_telat_hari_kerja($finger_masuk, $date." ".$jadwal["jam_masuk"]);
                }
                else{
            ...
            if(!$jadwal["id_jam_kerja"]) {
              ...
              if($jadwal['jumlah_finger'] < 1) {
                ...
                $finger_pulang       = $this->ambil_finger_pulang($date." ".$jadwal["jam_mulai_scan_masuk"], $date." ".$jadwal["jam_akhir_scan_pulang"], $id_pegawai)->tanggal;
                // $selisih_finger      = $this->ambil_selisih_menit($finger_masuk, $finger_pulang, $id_pegawai);
                //$finger_senin_jumat = $this->ambil_finger_masuk_pulang($date." ".$jadwal["jam_mulai_scan_masuk"], $date." ".$jadwal["jam_akhir_scan_pulang"], $id_pegawai);
                //$finger_masuk = $finger_senin_jumat->tanggal_masuk;
                //$finger_pulang = $finger_senin_jumat->tanggal_keluar;
                $selisih_finger = $this->ambil_selisih_menit($finger_masuk, $finger_pulang, $id_pegawai);
