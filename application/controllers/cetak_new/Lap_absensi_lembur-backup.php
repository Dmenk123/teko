...
class lap_absensi_lembur extends CI_Controller {
	...
	function cekMigrasiKominfo(){
		...
		$queryInstansi	=	"
			...
			from
				...
				pukh ON true
			where
				 pukh.kode_instansi = '5.16.00.00.00' and kode_status_pegawai = '1'";
				 // 3.02.00.00.00

		$data_pegawai = $this->db->query($queryInstansi)->result();
		foreach($data_pegawai as $temp){
			$begin = new DateTime( "2018-12-01" );
			$end   = new DateTime( "2018-12-09" );
	...
	function MigrasiPerbagian_insert01(){
		...
		foreach($dataDinas as $temp_dinas){
			...
			$tsch['finish_at'] = date('Y-m-d H:i:s');
			$tsch['status'] = 'Y';
	...
	function MigrasiPerbagian_InsertManual(){
		...
		$instansiRaw 	  = $this->input->post("idTarik");
	...
	function cekMigrasiKominfo_instansi(){
		...
		$queryInstansi	=	"
			...
			from
				...
				pukh ON true
			where
				 pukh.kode_instansi = '1.02.01.00.00' and kode_status_pegawai = '1'";
