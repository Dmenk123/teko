...
class Lap_skor2 extends CI_Controller {
	...
	public function index(){
		...
		foreach($this->dataPegawai as $dataPegawai){
			...
			$this->dataTable .= "<td align='center'>".$dataPegawai->nama_golongan."</td>";
					...
					from
						...
						LEFT JOIN LATERAL (
							...
							FROM
								m_pegawai_unit_kerja_histori h
								LEFT JOIN m_unit_organisasi_kerja muok ON  h.kode_unor =  muok.kode
								LEFT JOIN m_instansi mi ON  muok.kode_instansi = mi.kode WHERE h.tgl_mulai <= '2019-01-01' and m.id = h.id_pegawai
							ORDER BY h.tgl_mulai DESC LIMIT 1
						)
						pukh ON true
					where
						pukh.kode_instansi = '5.09.00.93.00' and m.id = '".$dataPegawai->id_pegawai."'
					order by
						m.nip"
				);
				$cek_posisi_unor = $cekDispendik->result();
			}else{
				$cek_posisi_unor = true;
			}
	...
	public function printed($hari_kerja, $bulan, $tahun, $id_instansi, $pns) {
		...
		if($this->input->get("type") == 'pdf') {
			...
			$html_body = $this->load->view('cetak/skor_new/body', $data_arr, true); //render the view into HTML
			...
			$this->load->view('cetak/skor_new/excel', $data_arr);

			ob_end_clean();
		}
		else {
			$this->load->view('cetak/skor_new/body', $data_arr);
		}
	}
