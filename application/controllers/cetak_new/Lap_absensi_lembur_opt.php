...
class lap_absensi_lembur_opt extends CI_Controller {
	...
	function cekdanricek(){
		// echo 'lontong';exit;
		//ini_set('display_errors', 1);
		$this->load->model('monitoring_tarik_model');
		$this->load->library(['migrasi_data_opt', 'linebot']);
		// $inisial  		  = $this->input->get("inisial");
		// $kategori  		  = $this->input->get("kategori");
		$dataInstansi = "select * from m_instansi WHERE kode = '5.16.00.00.00'";

		$dataDinas = $this->db->query($dataInstansi)->result();
		// echo '<pre>' , var_dump($dataDinas) , '</pre>';exit;
		foreach($dataDinas as $temp_dinas){
			$kode_dinas = $temp_dinas->kode;
			$nama_dinas = $temp_dinas->nama;

			if ($temp_dinas->kode == '5.09.00.93.00') {
				$queryInstansi = "
					select
						m.id,m.nama, m.nip, m.id_line, m.kode_jenis_kelamin, m.nama
					from
						m_pegawai m
						LEFT JOIN LATERAL (
						SELECT
						...
						LEFT JOIN m_instansi mi ON  muok.kode_instansi = mi.kode WHERE h.tgl_mulai <= '".date('Y-m-d')."' and m.id = h.id_pegawai and h.excel = 't' 
			...
			if ($data_pegawai) {
				foreach($data_pegawai as $list_peg){
					$select        = "tgl_meninggal, id, nama, meninggal";
					$where         = "id = '".$list_peg->id."' ";
					$dt_pegawai = $this->pegawai_model->getData3($select, $where);

					$end 	 = new DateTime(date("Y-m-d"));
					$begin   = new DateTime(date('Y-m-d', strtotime("-1 day")));

					$akhir = date("Y-m-d");
					$mulai = date('Y-m-d', strtotime("-1 day", strtotime($akhir)));
					// echo '<pre>' , var_dump($end, $begin) , '</pre>';exit;
					$dt_mulai = $mulai." 00:01";
					$dt_akhir = $akhir." 23:59";
					$arr_id_line = json_decode($list_peg->id_line, true);
					
					$queryCekDataMentah = "select tanggal from data_mentah where tanggal >= '".$mulai."' and tanggal <= '".$akhir."' and id_pegawai  = '".$list_peg->id."'";
					...
					$query_jadwal	= "
						...
						LEFT JOIN LATERAL (
							...
							WHERE tgl_mulai <= tgl AND id_pegawai = '".$list_peg->id."'
						...
						LEFT JOIN t_roster as tr ON tr.id_pegawai = '".$list_peg->id."' AND tr.tanggal = tgl::date
						...
						LEFT JOIN (
							...
							AND '".$dt_akhir."' 
							AND badgenumber || id_mesin IN ( SELECT user_id || id_mesin FROM mesin_user WHERE id_pegawai = '".$list_peg->id."' ) 
							GROUP BY
						...
						LEFT JOIN (
							...
							WHERE
								...
								and absensi_log.badgenumber || absensi_log.id_mesin in (SELECT user_id || id_mesin from mesin_user where id_pegawai = '".$list_peg->id."')
					...
					for($i = $begin; $i <= $end; $i->modify('+1 day')){
						...
						//jika data ada update, tidak insert
						if($dt_mentah){
							$this->migrasi_data_opt->cek_ulang_data_mentah(
								$i->format("Y-m-d"), $list_peg->id, "update", false, $lbr, $iz, $jdwl, $dt_pegawai->meninggal, $dt_pegawai->tgl_meninggal
							);
						}
						else{
							$this->migrasi_data_opt->cek_ulang_data_mentah(
								$i->format("Y-m-d"), $list_peg->id, "insert", false, $lbr, $iz, $jdwl, $dt_pegawai->meninggal, $dt_pegawai->tgl_meninggal
							);
						}
					}

					if (count($arr_id_line) > 0) {
						$post_line = $this->linebot->post($arr_id_line, $list_peg->id, $list_peg->kode_jenis_kelamin, $list_peg->nama, $end);
		            }
				}
			...
			// INSERT KODE DINAS
			echo "KODE INSTANSI : $kode_dinas </h1> <h1>NAMA INSTANSI : $nama_dinas", "\n";
	...
	/* function Coba4(){
		...
		$dataInstansi = "select * from m_instansi WHERE
							...
							AND nama NOT LIKE 'SMPN %'
							AND nama NOT LIKE 'TK %'
							AND nama NOT LIKE 'TKL%' order by nama asc limit 82 offset 280";
