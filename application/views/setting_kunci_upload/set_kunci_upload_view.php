	...
	<div class="portlet-body">
		<div class="row">
			<div class="col-xs-12"  id="data_list">
				...
				<?php if ($this->session->userdata('id_kategori_karyawan') == '2') { ?>
...
<script>
	...
	$(document).ready(function($) {
		...
		table = $('#dynamic-table').DataTable({ 
			...
			"ajax": {
				"url": "<?php echo site_url('setting_kunci_upload/get_data')?>",
				"type": "POST"
			},
	    ...
	    $('#proses-ceklis').on('click', function(event) {
	    	...
	    	$('#loading').modal('show');
	    	$.ajax({
	    		url: base_url + 'setting_kunci_upload/simpan_data',
	    		type: 'POST',
	    		dataType: 'json',
	    		data: $('#form-ceklis').serialize(),
	    		success : function(response){
	    			if (response.status) {
	    				alert('Berhasil Setting Laporan');
	    				$('#loading').modal('hide');
	    ...
	    /*$('#proses-ceklis2').on('click', function(event) {
	    	$('#loading').modal('show');
	    });*/
