	...
	<div class="portlet-body">
		<div class="row">
			<form class="form-horizontal"  method="get">
				...
				<div class="form-group">
					<label class="control-label col-sm-2" >NIP / Nama Pegawai :</label>
		...
		?>
			<script>
				window.open('<?=base_url();?>cetak_new/lap_per_pegawai/?bulan=<?php echo $this->input->get("bulan"); ?>&tahun=<?php echo $this->input->get("tahun"); ?>&id_instansi=<?php echo $this->input->get("id_instansi"); ?>&id_pegawai=<?php echo $this->input->get("id_pegawai"); ?>')
			</script>
		<?php
			}
		}
...
<script>
    ...
    $('.tarik').click(function(e) {
    	e.preventDefault();
    	var idPeg = $('#id_pegawai').val();
    	$('.tarik').attr('disabled', true);
		$('#btn-submit').attr('disabled', false);
    	//alert($('#id_pegawai').val());
		$.ajax({
     		url:  base_url + 'cetak_new/lap_absensi_lembur/UpdatePerPegawai',
     		type: 'POST',
     		dataType: 'json',
     		data: {id_pegawai: idPeg},
