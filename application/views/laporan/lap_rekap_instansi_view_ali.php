<div class="portlet box purple">
	...
	<div class="portlet-body">
		...
		?>
			<script>
				window.open('<?=base_url();?>cetak_new/lap_rekap_instansi2/?bulan=<?php echo $this->input->get("bulan"); ?>&tahun=<?php echo $this->input->get("tahun"); ?>&id_instansi=<?php echo $this->input->get("id_instansi"); ?>&pns=<?php echo $this->input->get("pns"); ?>&type=<?php echo $this->input->get("type"); ?>')
			</script>
		<?php
		}
		?>
...
<script>
	...
	function proses_generate() {
		...
		$.ajax({
			url:  base_url + 'cetak_new/lap_rekap_instansi/generate',
			type: 'get',
			dataType: 'json',
			data: data,
			success: function(data){
				if (data.status == 'gagal') {
					...
					$('.anchor-stop-upd').attr("href", base_url + "cetak_new/lap_rekap_instansi2/stop?bulan="+$('#bulan').val()+"&tahun="+$('#tahun').val()+"&id_instansi="+$('#id_instansi').val()+"&pns="+$('#pns').val());
	...
	function proses_generate_peg(i, i_max, data_kirim) {
		...
		$.ajax({
			url:  base_url + 'cetak_new/lap_rekap_instansi/proses_generate_perpegawai',
			type: 'POST',
			dataType: 'json',
			data: data_pegawai,
			success: function(data){
				...
				else
				{
					...
					{
						$.ajax({
							url:  base_url + 'cetak_new/lap_rekap_instansi/update_selesai_gen_laporan',
							type: 'POST',
							dataType: 'json',
							data: {
							...
							success: function(data){
								if (data.status = 'sukses') {
									...
									$.alert({
										...
										type: 'green',
										title: 'SUKSES',
										content: 'Data laporan rekap instansi berhasil di update !',
										buttons: {
									        somethingElse: {
									            text: 'Ok',
									            btnClass: 'btn-blue',
									            ...
									            action: function(){
									                ...
									                $("#btnGenerate").prop('disabled', false);
													$('#btnTampilkan').prop('disabled', false);
									                window.open(base_url + 'cetak_new/lap_rekap_instansi2?bulan='+data_pegawai.bulan+'&tahun='+data_pegawai.tahun+'&id_instansi='+data_pegawai.id_instansi_peg+'&pns='+data_pegawai.pns+'');
									            }
									        }
									    }
									});
