<div class="portlet box purple">
	...
	<div class="portlet-body">
			...
			<form class="form-horizontal"  method="get">
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						<select type="input" class="form-control select2" onchange="ganti_instansi(this.value)" required id="id_instansi"  name="id_instansi">
							<option value="">Silahkan Pilih</option>
							<option value="5.16.00.00.00" selected>Dinas Komunikasi dan Informatika</option>
						</select>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-6">
						<select type="input" class="form-control select2" id="id_bidang"  name="id_bidang">
							<option value="">Silahkan Pilih</option>
							<?php foreach ($this->dataBidang as $key => $bdg) { ?>
								<option <?php if($this->input->get('id_bidang') == $bdg['kode']) echo "selected"; ?> value=<?php echo $bdg['kode']; ?>> <?php echo $bdg['nama']; ?> </option>
							<?php } ?>
						</select>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						...
						<?php if($this->session->userdata('kategori_karyawan') != 'BPK' && $this->session->userdata('id_kategori_karyawan') != '3' && $this->session->userdata('id_kategori_karyawan') != '4') { ?>
		...
		?>
			<script>
				window.open('<?=base_url();?>cetak_new/Lap_skor2_kpi/?bulan=<?php echo $this->input->get("bulan"); ?>&tahun=<?php echo $this->input->get("tahun"); ?>&id_instansi=<?php echo $this->input->get("id_instansi"); ?>&id_bidang=<?php echo $this->input->get("id_bidang"); ?>&pns=<?php echo $this->input->get("pns"); ?>&type=<?php echo $this->input->get("type"); ?>')
			</script>
		<?php
		}
		?>
...
<script>
	...
	function proses_generate() {
		...
		$.ajax({
			url:  base_url + 'cetak_new/lap_skor2_kpi/generate_update',
			type: 'get',
			dataType: 'json',
			data: data,
	...
	function proses_generate_peg(i, i_max, data_kirim) {
		...
		var data_pegawai = {
			...
			id_unor 		: data_kirim.pesan[i].kode_unor,
		...
		$.ajax({
			url:  base_url + 'cetak_new/lap_skor2_kpi/proses_generate_perpegawai',
			type: 'POST',
			dataType: 'json',
			data: data_pegawai,
			success: function(data){
				...
				else 
				{
					...
					{
						$.ajax({
							url:  base_url + 'cetak_new/lap_skor2_kpi/update_selesai_gen_laporan',
							type: 'POST',
							dataType: 'json',
							data: {
								...
								id_unor_update 	: data_pegawai.id_unor,
										...
										buttons: {
									        somethingElse: {
									            ...
									            action: function(){
																...
																$("#btnGenerate").prop('disabled', false);
																$('#btnTampilkan').prop('disabled', false);
																window.open(base_url + 'cetak_new/lap_skor2_kpi?bulan='+data_pegawai.bulan+'&tahun='+data_pegawai.tahun+'&id_instansi='+data_pegawai.id_instansi_peg+'&id_bidang='+data_pegawai.id_unor+'&pns='+data_pegawai.pns+'');
									            }
									        }
									    }
									});
