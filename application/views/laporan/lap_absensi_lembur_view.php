<div class="portlet box purple">
	...
	<div class="portlet-body">
		...
		?>
			<script>
				window.open('<?=base_url();?>cetak_new/lap_absensi_lembur2/?bulan=<?php echo $this->input->get("bulan"); ?>&tahun=<?php echo $this->input->get("tahun"); ?>&id_instansi=<?php echo $this->input->get("id_instansi"); ?>&pns=<?php echo $this->input->get("pns"); ?>&type=<?php echo $this->input->get("type"); ?>')
			</script>
		<?php
		}
		?>
    ...
    <div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title">Proses Update Laporan Absensi Lembur, Mohon menunggu.</h4>
			</div>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
		...
		?>
		<a href="<?= base_url('cetak_new/lap_absensi_lembur2?bulan='.$bulan_get.'&tahun='.$tahun_get.'&id_instansi='.$id_instansi_get.'&pns='.$pns_get.'&lanjut_cetak=1') ?>" class="btn btn-danger">Lanjutkan Cetak</a>
...
<script>
	...
	function proses_generate() {
		...
		$.ajax({
			url:  base_url + 'cetak_new/lap_absensi_lembur/generate',
			type: 'get',
			dataType: 'json',
			data: data,
			success: function(data){
				if (data.status == 'gagal') {
					...
					$('.anchor-stop-upd').attr("href", base_url + "cetak_new/lap_absensi_lembur2/stop?bulan="+$('#bulan').val()+"&tahun="+$('#tahun').val()+"&id_instansi="+$('#id_instansi').val()+"&pns="+$('#pns').val());
	...
	function proses_generate_peg(i, i_max, data_kirim) {
		...
		//console.log(data);
		$.ajax({
			url:  base_url + 'cetak_new/lap_absensi_lembur/proses_generate_perpegawai',
			type: 'POST',
			dataType: 'json',
			data: data_pegawai,
			success: function(data){
				...
				else
				{
					...
					{
						$.ajax({
							url:  base_url + 'cetak_new/lap_absensi_lembur/update_selesai_gen_laporan',
							type: 'POST',
							dataType: 'json',
							data: {
							...
							success: function(data){
								if (data.status = 'sukses') {
									...
									$.alert({
										...
										type: 'green',
										title: 'SUKSES',
										content: 'Data laporan absensi lembur sukses di update !',
										buttons: {
									        somethingElse: {
									            text: 'Ok',
									            btnClass: 'btn-blue',
									            ...
									            action: function(){
									                ...
									                $("#btnGenerate").prop('disabled', false);
													$('#btnTampilkan').prop('disabled', false);
									                window.open(base_url + 'cetak_new/lap_absensi_lembur2?bulan='+data_pegawai.bulan+'&tahun='+data_pegawai.tahun+'&id_instansi='+data_pegawai.id_instansi_peg+'&pns='+data_pegawai.pns+'');
									            }
									        }
									    }
									});
