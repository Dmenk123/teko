...
class Pegawai_model extends CI_Model {
	...
	function showData2Log($where, $tgl_mulai, $tgl_akhir){
		$tglNow =	date('Y-m-d');

		if($tgl_akhir <> null) {
			$where2 = "h.tgl_mulai <= '".$tgl_akhir."'";
		}
		else if($tgl_mulai <> null) {
			$where2 = "h.tgl_mulai <= '".$tgl_mulai."'";
		}
		else {
			$where2 = "h.tgl_mulai <= '".$tglNow."'";
		}
		...
		from
			...
			LEFT JOIN LATERAL (
				...
				FROM
					m_pegawai_unit_kerja_histori h
					LEFT JOIN m_unit_organisasi_kerja muok ON  h.kode_unor =  muok.kode
					LEFT JOIN m_instansi mi ON  muok.kode_instansi = mi.kode WHERE $where2 and m.id = h.id_pegawai
				ORDER BY h.tgl_mulai DESC LIMIT 1
			)
			pukh ON true
		where
	...
	function showData2($where, $limit=null){
	...
	function getCount2($where, $limit=null){
		$tglSelesai		=	date('Y-m-d');
	...
	function getData3($select, $where){
		$this->db->select($select);
		$this->db->where($where);
		return $this->db->get("m_pegawai")->row();
	}
